				...
				<div class="container">
					<div class="row">
						<div class="col-md-10 col-md-offset-1 text-center">
							<h1 class="hestia-title">Hello User			</h1>			</div>
					</div>
				</div>
			</div>
		</header>
...
while(rs.next()){
	String c1=rs.getString("email");
	String c2=rs.getString("password");
	if((e.equalsIgnoreCase(c1))&&(p.equals(c2))){
		temp=1;
		String c=rs.getString("course");
		String n=rs.getString("name");
		int t=rs.getInt("test");
		String s=rs.getString("score");
		//String url=c3+"test.jsp";
		//response.sendRedirect(url);
		session.setAttribute("username", n);
		session.setAttribute("password", p);
		session.setAttribute("useremail", e);
		session.setAttribute("usercourse", c);
		session.setAttribute("usertest", t);
		session.setAttribute("userscore", s);
	
		session.setAttribute("atemp", 10);
		int no=1;
		session.setAttribute("no",no);
		int marks=0;
		session.setAttribute("mar", marks);
%>
	<h2 style="text-align:center;">Welcome <%=n %> !</h2>
	<h3 style="text-align:center;">You have Chosen Language <%=c %></h3>
	<hr>
	<p style="text-align:left;">
	<h4>Number Of Test Given = <%=t %></h4> <br>
	<h4>Number Of Marks Scored = <%=s %></h4>
	<br>
	<hr><br>
	<div style="text-align: center;">
	<a href="test.jsp">
	<img src="answer-sheet-with-pencil.jpg" title="Start Test" alt="Start Test" style="width:200px;height:200px;"><br>Click Here to Start Test</a>
	</div>
	<br><br><hr>
<%
}
...
if(temp==0){
	out.println("<h3>Wrong Email Id or Password</h3>");
