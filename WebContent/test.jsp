				...
				<div class="container">
					<div class="row">
						<div class="col-md-10 col-md-offset-1 text-center">
							<h1 class="hestia-title">Select Option and Click Next			</h1>			</div>
					</div>
				</div>
			</div>
		</header>
...
int test=(Integer)session.getAttribute("usertest");
...
if(test<1){
	...
	Statement st=conn.createStatement();
		...
		if(table.equalsIgnoreCase("c")){
			ResultSet rs1=st.executeQuery("Select * from c");
			while(rs1.next()){
				session.setAttribute("no",no);
				if(10==atemp){
					String q=rs1.getString("question");
					String optiona=rs1.getString("a");
					String optionb=rs1.getString("b");
					String optionc=rs1.getString("c");
					String optiond=rs1.getString("d");
					int number=rs1.getInt("number");
					System.out.println("number " +number);
					session.setAttribute("numb",number);
					atemp--;
					tempc++;
					System.out.println("temp c in question"+tempc);
					System.out.println("atemp in question" +atemp);
					System.out.println("marks"+marks);
			%>
					 <form action="javatest.jsp" method="post">
				<br><br> <fieldset style="font-size: 20px">
					<p> <%= no%>.&ensp; <%=q %> </p><br>
					<p> <input type="radio" name="ans" value="a" > (A) &ensp; <%=optiona %></p>
					<p> <input type="radio" name="ans" value="b" > (B) &ensp; <%=optionb %></p>
					<p> <input type="radio" name="ans" value="c" > (C) &ensp; <%=optionc %></p>
					<p> <input type="radio" name="ans" value="d" > (D) &ensp; <%=optiond %></p><br>
			...
			<%		
					no++;
					System.out.println("no in question :"+no);
				}
				else{
						 atemp--;
					System.out.println("atemp outside question" +atemp);
					no++;

					System.out.println("no out question :"+no);
				...
				System.out.println("temp c out question"+tempc);
				if(tempc==0){
					out.println("<h3>Your Score</h3>");
				...
				out.println("<br>Percentage : "+ m);
				
				String Semail=(String)session.getAttribute("useremail");
				...
				PreparedStatement ps=con.prepareStatement("update student set score=?, test=? where email=?");
				...
				%><br><br>
				<div class="buttons">
										<a href="login.html" title="Finish"
				...
				con.close();
					...
					while(rs2.next()){
						...
						if(10==atemp){
							...
							String optiona=rs2.getString("a");
							String optionb=rs2.getString("b");
							String optionc=rs2.getString("c");
							String optiond=rs2.getString("d");
							...
							System.out.println("marks"+marks);
					%>
							 <br><br><form action="javatest.jsp" method="post">
						...
						con.close();
							...
							while(rs3.next()){
								...
								if(10==atemp){
									...
									String optiona=rs3.getString("a");
									String optionb=rs3.getString("b");
									String optionc=rs3.getString("c");
									String optiond=rs3.getString("d");
...
%>	
		<h3 style="text-align: center;">Test Limit Is Exceeded</h3><br><br>
		
		<div  style="text-align: center; font-size: 20px;">
		<a href="renew.jsp"><b><u>Request Renewal and Log-Out</u></b></a>
		</div>
<% 
}
